...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        closest_distance = 99999  # This might break the code, might need to be first distance in ghost list
        closest_ghost_position = None
        for distribution in livingGhostPositionDistributions:
            likely_ghost_pos = distribution.argMax()
            new_distance = self.distancer.getDistance(pacmanPosition, likely_ghost_pos)
            if new_distance < closest_distance:
                closest_distance = new_distance
                closest_ghost_position = likely_ghost_pos

        # Find best possible action to get us closer to that closest ghost
        output_action = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            new_distance = self.distancer.getDistance(successorPosition, closest_ghost_position)
            if new_distance < closest_distance:
                closest_distance = new_distance
